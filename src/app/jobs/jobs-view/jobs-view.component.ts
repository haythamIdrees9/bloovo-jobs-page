...
export class JobsViewComponent implements OnInit {
  ...
  private onTermChange(term: string) {
    this.term = term;
    if (term) {
  ...
  private handleUrlChange() {
    this.route.queryParams.subscribe((queryParams) => {
      this.onUrlChange(queryParams);
    });
  }

  private onUrlChange(queryParams: Params = this.route.snapshot.queryParams) {
  ...
  private handleFilterersValue() {
    this.filteredJobs = this.filteredJobs.filter((job: jobModel) => {
      if (
        this.isFilterInclude('sectors', this.sectors, job.sector) ||
        this.isFilterInclude('countries', this.countries, job.country) ||
